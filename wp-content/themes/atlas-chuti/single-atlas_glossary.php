...
while ( have_posts() ) :
	...
	<section class="container-narrow" style="padding:8px var(--gutter) 0;display:flex;flex-direction:column;gap:36px;">
		<?php if ( $detailed ) : ?><div><h2 style="font-size:24px;">Detailní vysvětlení</h2><div><?php echo wp_kses_post( wpautop( $detailed ) ); ?></div></div><?php endif; ?>
		<?php if ( $taste ) : ?><div><h2 style="font-size:24px;">Jak chutná</h2><p style="font-size:15px;color:var(--text-soft);line-height:1.65;"><?php echo esc_html( $taste ); ?></p></div><?php endif; ?>
		<?php if ( $usage ) : ?><div><h2 style="font-size:24px;">Jak se používá</h2><p style="font-size:15px;color:var(--text-soft);line-height:1.65;"><?php echo esc_html( $usage ); ?></p></div><?php endif; ?>
		<?php if ( $substitute ) : ?><div><h2 style="font-size:24px;">Čím ho případně nahradit</h2><p style="font-size:15px;color:var(--text-soft);line-height:1.65;"><?php echo esc_html( $substitute ); ?></p></div><?php endif; ?>
		<?php if ( $country_ids ) : ?>
			<div>
				<h2 style="font-size:24px;">Používá se v</h2>
	...
	if ( $recipes ) :
		...
		<section class="container section" style="padding-top:56px;">
			<h2>Recepty s pojmem „<?php the_title(); ?>“</h2>
